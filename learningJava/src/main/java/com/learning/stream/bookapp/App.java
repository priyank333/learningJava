...
public class App {
  public static void main(String[] args) {
    ...
    System.out.println(
        "List of books which type is Novel by :: filter(x -> x.getType().equals(Type.NOVEL))");
    List<Book> novelBooks =
        books.stream().filter(x -> x.getType().equals(Type.NOVEL)).collect(Collectors.toList());
    novelBooks.forEach(System.out::println);

    // Get list of books those types are novel in sorted manner

    List<Book> novelSortedBooksByAuthor =
        books.stream()
    ...
    Map<Type, List<Book>> groupByType =
