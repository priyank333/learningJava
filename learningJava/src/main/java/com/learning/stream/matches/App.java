...
public class App {
  public static void main(String[] args) {
    ...
    /**
     * noneMatch() -> checking if predicate not matches all elements No need to consider all list
     * because if any single item is found not match it return false. Short-Circuiting: Here no need
     * to process whole list
     */
    isThere = books.stream().noneMatch(b -> b.getPages() > 2000);
    System.out.println("Book pages with > 2000 (NoneMatch): " + isThere);

    /**
     * findAny() : Search from multiple sublist and if it finds any object, it breaks the process
     * and return. Short-Circuiting: Here no need to process whole list
     */
