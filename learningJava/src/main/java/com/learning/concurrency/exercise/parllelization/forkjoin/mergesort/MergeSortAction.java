...
public class MergeSortAction extends RecursiveAction {
  ...
  private void processSubTasks(int middleIndex) {
    ...
    MergeSortAction subTask1 = new MergeSortAction(leftSubArray);
    int[] rightSubArray = Arrays.copyOfRange(this.list, middleIndex, this.list.length);
    MergeSortAction subTask2 = new MergeSortAction(rightSubArray);
  ...
  private void merge(int[] leftSubarray, int[] rightSubarray, int[] originalArray) {
    ...
    while (i < leftSubarray.length && j < rightSubarray.length) {
      if (leftSubarray[i] < rightSubarray[j]) originalArray[k++] = leftSubarray[i++];
      else originalArray[k++] = rightSubarray[j++];
    }

    while (i < leftSubarray.length) originalArray[k++] = leftSubarray[i++];

    while (j < rightSubarray.length) originalArray[k++] = rightSubarray[j++];
  }
