...
public class ProcessListAction extends RecursiveAction {
  ...
  private List<ProcessListAction> createSubTask() {
    ...
    ProcessListAction part1 = new ProcessListAction(this.list.subList(0, this.list.size() / 2));
    ProcessListAction part2 =
        new ProcessListAction(this.list.subList(this.list.size() / 2, this.list.size()));
    subTasks.add(part1);
    subTasks.add(part2);
    return subTasks;
  }
